...
if uploaded_file:
    ...
    with st.spinner("Summarizing your resume..."):
        summary = ask_openai(f"Summarize the following resume highlighting the skills, achievements, education, projects and experiences: \n\n{resume_text}", max_tokens=500)
    ...
    with st.spinner("Creating Future Roadmap..."):
        roadmap = ask_openai(f"Based on the following resume, suggest a future roadmap to improve this persons's career prospects (Skill to learn, certification needed, industry exposure): \n\n{resume_text}", max_tokens=400)
    ...
    if st.button("🔎 Get Job Recommendations"):
        with st.spinner("Fetching job recommendations..."):
            keywords = ask_openai(f"Based on this resume summary, suggest the best job titles and keywords for searching jobs. Give a comma-separated list only, no explanation.: \n\n{summary}", max_tokens=100)
